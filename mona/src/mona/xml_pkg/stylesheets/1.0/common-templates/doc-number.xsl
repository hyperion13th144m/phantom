...
<xsl:stylesheet version="3.0"
    ...
    <xsl:template name="translate-application-number">
        ...
        <xsl:choose>
            <xsl:when test="string-length($number) != 10">
                <xsl:text>Illegal format application number</xsl:text>
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>
    ...
    <xsl:template name="translate-registered-number">
        ...
        <xsl:choose>
            <xsl:when
                ...
                <xsl:text>Illegal registered number length</xsl:text>
            </xsl:when>
            <xsl:when
    ...
    <!-- 公告番号の変換
      ...
      -->
    <xsl:template name="translate-examined-pub-number">
        <xsl:param name="number" as="xs:string" />
        <xsl:param name="law" as="xs:string" />
