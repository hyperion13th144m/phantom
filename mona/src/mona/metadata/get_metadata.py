...
from ..default_config import SCHEMA_VER
...
def get_metadata(archive_path: str) -> Metadata:
    ...
    m.add_metadata(
        "root",
        {
            "docId": detect_document_id(archive_path),
            "archive": Path(archive_path).name,
            "ext": detect_document_extension(archive_path),
            "kind": detect_document_kind(archive_path),
