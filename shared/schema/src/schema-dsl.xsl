<xsl:stylesheet
    ...
    <xsl:template match="/"> { "$schema": "http://json-schema.org/draft/2020-12/schema", "$defs": { <xsl:for-each
    ...
    <xsl:template match="schema:object" mode="schema-object"> { "type": "object", "properties": { <xsl:apply-templates
