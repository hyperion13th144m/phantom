...
all_codes = [code for group in code_config.values() for code in group["codes"].keys()]
all_categories = list(code_config.keys())


def get_all_document_codes() -> List[str]:
    return all_codes + all_categories + ["ALL"]


def is_valid_document_code(codes: List[str]) -> bool:
    # empty list is valid (means no filter)
    return all(c in all_codes or c in all_categories or c == "ALL" for c in codes)


def get_target_document_codes(codes: List[str]) -> List[str]:
    results = set()
    ...
    for c in codes:
        if c == "ALL":
            ...
            results.update(code_config[c]["codes"].keys())
        elif c in all_codes:
            results.add(c)
        else:
            pass
