...
def get_test_cases():
    ...
    mp_files = sorted(test_data_root.rglob("manifest.json"))

    for mp_file in mp_files:
        mp_dir = mp_file.parent
        xml_files = sorted(mp_dir.glob("xml/*.xml"))
        json_dir = mp_dir / "xml-to-json"

        if xml_files and json_dir.exists():
            # Create test case name from directory path
            relative_path = mp_dir.relative_to(test_data_root)
...
@pytest.mark.parametrize(
    "test_name,xml_files,expected_json_dir",
    get_test_cases(),
...
def test_translate_all(test_name, xml_files, expected_json_dir):
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        assert len(generated_json_files) > 0, f"No JSON files generated in {output_dir}"
