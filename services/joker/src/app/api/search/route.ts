...
export async function GET(req: NextRequest) {
  ...
  const parsed = QuerySchema.safeParse({
    ...
    specialMentionMatterArticle:
      url.searchParams.get("specialMentionMatterArticle") ?? undefined,
    rejectionReasonArticle:
      url.searchParams.get("rejectionReasonArticle") ?? undefined,
    priorityClaims: url.searchParams.get("priorityClaims") ?? undefined,
    withHighlight:
      url.searchParams.get("withHighlight") === "true" ? true : undefined,
  });
  if (!parsed.success) {
  ...
  if (specialMentionMatterArticle)
    filter.push({
      match: { specialMentionMatterArticle: specialMentionMatterArticle },
    });
  if (rejectionReasonArticle)
    filter.push({ match: { rejectionReasonArticle: rejectionReasonArticle } });
  if (priorityClaims)
    filter.push({ match: { priorityClaims: priorityClaims } });
  ...
  try {
    const result: estypes.SearchResponse<PatentDocumentSource> =
      await es.search({
    ...
    const aggregations = {
      applicants:
        ...
        Array.isArray(
          (
            result.aggregations
              .applicants as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
                .applicants as estypes.AggregationsStringTermsAggregate
            ).buckets as Array<{ key: string; doc_count: number }>)
          : [],
      inventors:
        result.aggregations &&
        Array.isArray(
          (
            result.aggregations
              .inventors as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
      ...
      assignees:
        ...
        Array.isArray(
          (
            result.aggregations
              .assignees as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
      ...
      tags:
        result.aggregations &&
        Array.isArray(
          (result.aggregations.tags as estypes.AggregationsStringTermsAggregate)
            ?.buckets,
        )
          ? ((
              result.aggregations
                .tags as estypes.AggregationsStringTermsAggregate
            ).buckets as Array<{ key: string; doc_count: number }>)
          : [],
      documentNames:
        result.aggregations &&
        Array.isArray(
          (
            result.aggregations
              .documentNames as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
      ...
      specialMentionMatterArticle:
        ...
        Array.isArray(
          (
            result.aggregations
              .specialMentionMatterArticle as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
      ...
      rejectionReasonArticle:
        ...
        Array.isArray(
          (
            result.aggregations
              .rejectionReasonArticle as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
      ...
      priorityClaims:
        ...
        Array.isArray(
          (
            result.aggregations
              .priorityClaims as estypes.AggregationsStringTermsAggregate
          )?.buckets,
        )
          ? ((
              result.aggregations
